...
export default function AdminsPage() {
  ...
  const [editForm, setEditForm] = useState<EditAdminForm>({ email: "", password: "", role: "subadmin" });
  ...
  const handleSaveEdit = async () => {
    ...
    if (editForm.email && editForm.email !== (editingAdmin.email ?? "")) body.email = editForm.email;
    if (editForm.password) body.password = editForm.password;
    if (editForm.role !== editingAdmin.role) body.role = editForm.role;

    const res = await fetch(`/api/users/${editingAdmin.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div>
      ...
      {/* Edit Admin Modal */}
      <Modal open={!!editingAdmin} onClose={() => setEditingAdmin(null)}>
        <h3 className="text-lg font-semibold mb-4">{t.admin.editAdmin}</h3>
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            <label className="text-sm text-[#B3B3B3]">{t.admin.email}</label>
          ...
          <div className="flex flex-col gap-1">
            <label className="text-sm text-[#B3B3B3]">{t.admin.password}</label>
            <div className="relative">
              <input type={showPw ? "text" : "password"} value={editForm.password} onChange={(e) => setEditForm((f) => ({ ...f, password: e.target.value }))} placeholder="••••••••" className={inputClass + " pr-10"} />
              <button type="button" onClick={() => setShowPw((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#B3B3B3] hover:text-white">
                {showPw ? (
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                ) : (
          ...
          <div className="flex flex-col gap-1">
            <label className="text-sm text-[#B3B3B3]">{t.admin.role}</label>
            <select value={editForm.role} onChange={(e) => setEditForm((f) => ({ ...f, role: e.target.value as "admin" | "subadmin" }))} className={inputClass}>
              <option value="subadmin">Subadmin</option>
              <option value="admin">Admin</option>
            </select>
          </div>
          {error && <p className="text-red-400 text-sm">{error}</p>}
          <div className="flex justify-end gap-3 mt-2">
            <button onClick={() => setEditingAdmin(null)} className="text-sm text-[#B3B3B3] hover:text-white">{t.admin.cancel}</button>
