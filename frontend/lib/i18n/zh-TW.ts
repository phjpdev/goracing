export const zhTW = {
  // Header
  header: {
    manage: "管理",
    logout: "登出",
    roleMember: "會員",
    roleAdmin: "管理員",
    roleSubadmin: "副管理員",
    records: "記錄",
  },

  nav: {
    home: "首頁",
    matches: "賽事",
    records: "紀錄",
    member: "會員",
    contact: "聯繫我們",
    manageMembers: "管理會員",
  },

  // Hero / Landing
  hero: {
    title: "賽馬智能分析",
    description: "將複雜的賽馬數據轉化為清晰、自信的決策，專為現代投注者打造的即時AI分析。",
    viewMatches: "查看即場賽事",
    login: "登入",
    liveRacesToday: "今日即場賽事",
    modelAccuracy: "模型預測準確度",
  },

  // Feature cards
  features: {
    aiIntelligence: "先進的 AI 賽馬分析洞見",
    aiIntelligenceDesc: "我們的尖端演算法綜合分析馬匹過往表現、場地狀況、騎師練馬師數據，以及即時市場趨勢，在賽事開始前為你提供準確且實用的分析見解。",
    smartRacecard: "智能賽馬卡體驗",
    smartRacecardDesc: "全新設計的賽馬卡，將賠率、表現趨勢、速度數據及馬匹分析整合成簡潔、直觀、隨時可決策的介面。",
    liveOdds: "預測智能系統",
    liveOddsDesc: "以數據為基礎的勝出、位置及連贏概率，會隨即時賠率變化、場地偏好及天氣狀況即時調整。",
    insights: "賽前預見賽事",
    insightsDesc: "先進的賽事模擬引擎，讓你模擬不同比賽情境，清楚了解步速、檔位、場地狀況及騎法如何影響最終結果。",
  },

  // Visual Analytics section
  visualAnalytics: {
    title: "視覺化分析",
    subtitle: "強大的視覺化工具將原始賽馬數據轉化為一目了然、信心十足的決策依據。",
    heatmaps: "季節熱圖分析",
    heatmapsDesc: "透過季節、距離、場地及天氣條件的熱圖，輕鬆發現長期隱藏的表現規律，找出優勢所在。",
    marketActivity: "即時市場動向",
    marketActivityDesc: "實時監測賠率波動、投注動能及市場信心變化，幫助你捕捉價值投注及大戶資金流向。",
    pedigree: "血統雷達分析",
    pedigreeDesc: "互動式血統優勢與弱點視覺化分析，協助評估馬匹對不同距離、場地及賽事條件的適合程度。",
  },

  // Connect section
  connect: {
    title: "聯絡Go Racing",
    description: "有任何問題或需要專業支援嗎？我們的團隊隨時準備協助你探索賽馬分析之旅。歡迎隨時聯絡我們。",
  },

  // Matches page
  matches: {
    liveOddsMatrix: "即場賠率矩陣",
    horse: "馬匹",
    odds: "賠率",
    trend: "走勢",
    aiPct: "AI%",
    speed: "速度",
    ev: "EV",
    action: "操作",
    clickHere: "查看",
    selectRace: "請選擇賽事查看賠率",
    match: "場次",
    frontRunnerBias: "前領馬偏向",
    detected: "偵測到",
    class: "級別",
    track: "場地",
    duration: "開跑時間",
    winRate: "勝率",
    viewDetails: "查看詳情",
    races: "場賽事",
    noMeeting: "此日期及場地沒有賽事。",
    loadingRaces: "載入賽事中…",
  },

  // Race analysis page
  races: {
    back: "返回",
    smartRacecard: "智能馬卡",
    winPercentage: "勝率分析",
    goldHighlight: "精選推介",
    loadingRaceData: "載入賽事資料中…",
    rank: "排名",
    jockey: "騎師",
    trainer: "練馬師",
    turf: "跑道",
    winPct: "勝率 %",
    betStatus: "投注狀態",
    closed: "已截止",
    accepting: "接受中",
    pedigreeAnalysis: "血統分析",
    aiWinProbability: "AI 勝出概率",
    winChance: "勝出機會",
    marketActivity: "市場活動",
    oddsDropping: "賠率下降",
    oddsDrifting: "賠率上升",
    stable: "穩定",
    upcoming: "即將開跑",
    live: "進行中",
    finished: "已完賽",
    aiAnalyzing: "AI 正在分析此賽事…",
    retry: "重試",
    missingContext: "缺少賽事資料，請返回賽事列表。",
    raceNotFound: "找不到賽事",
    failedToLoad: "載入賽事資料失敗。",
    failedAi: "無法連接AI分析服務。",
  },

  // Auth
  auth: {
    loginTitle: "登入",
    loginWelcome: "歡迎回到賽馬智能分析平台",
    signupTitle: "建立帳號",
    signupWelcome: "加入我們的AI賽馬分析平台",
    username: "用戶名稱",
    email: "電郵地址",
    password: "密碼",
    confirmPassword: "確認密碼",
    referralSource: "哪一個地方看到廣告？",
    loginCta: "登入",
    signupCta: "註冊",
    haveAccount: "已有帳號？",
    noAccount: "沒有帳號？",
    privacyPrefix: "我同意",
    privacyPolicy: "私隱政策",
    loggingIn: "登入中…",
    creatingAccount: "建立帳號中…",
    signUp: "註冊",
    logIn: "登入",
    loginRequiredTitle: "請先登入",
    loginRequiredMessage: "登入後即可查看賽事分析結果。",
    loginRequiredCta: "前往登入",
    loginRequiredCancel: "稍後",
  },

  // Validation
  validation: {
    usernameRequired: "請輸入用戶名稱。",
    usernameInvalid: "用戶名稱須為3–30個字元，只可包含英文字母、數字及底線。",
    loginIdentifierInvalid: "請輸入有效的用戶名稱或電子郵件。",
    emailRequired: "請輸入電郵地址。",
    emailInvalid: "請輸入有效的電郵地址。",
    passwordRequired: "請輸入密碼。",
    passwordMin: "密碼須至少8個字元。",
    passwordUppercase: "密碼須包含大寫英文字母。",
    passwordNumber: "密碼須包含數字。",
    confirmRequired: "請確認密碼。",
    confirmMismatch: "密碼不一致。",
    privacyRequired: "您必須同意私隱政策才能繼續。",
    referralRequired: "請選擇廣告來源。",
  },

  privacyPolicyPage: {
    title: "私隱政策",
    lastUpdated: "最後更新：2026年6月",
    sections: [
      {
        heading: "1. 簡介",
        body: "Go Racing（「我們」）重視您的私隱。本私隱政策說明我們在您使用賽馬智能分析平台時，如何收集、使用及保護您的個人資料。",
      },
      {
        heading: "2. 我們收集的資料",
        body: "當您註冊帳號時，我們會收集您的用戶名稱及密碼（以加密形式儲存）。我們亦可能收集使用紀錄，例如登入時間、瀏覽頁面及使用分析功能的記錄，以改善服務質素。",
      },
      {
        heading: "3. 資料用途",
        body: "我們使用您的資料以建立及管理帳號、提供賽事分析服務、維持平台安全，以及遵守適用的法律要求。我們不會將您的個人資料出售予第三方。",
      },
      {
        heading: "4. 資料安全",
        body: "我們採取合理的技術及組織措施保護您的資料，包括加密密碼及安全的伺服器環境。然而，互聯網傳輸無法保證絕對安全。",
      },
      {
        heading: "5. Cookie",
        body: "我們使用 Cookie 及類似技術以維持您的登入狀態及記住語言偏好。您可透過瀏覽器設定管理 Cookie，但部分功能可能因此無法正常運作。",
      },
      {
        heading: "6. 您的權利",
        body: "您有權查閱、更正或要求刪除您的個人資料。如有相關要求，請透過本平台提供的聯繫方式與我們聯絡。",
      },
      {
        heading: "7. 年齡限制",
        body: "本平台僅供年滿18歲人士使用。我們不會故意收集未滿18歲人士的個人資料。",
      },
      {
        heading: "8. 政策更新",
        body: "我們可能不時更新本私隱政策。更新後的版本將於本頁公布，繼續使用服務即表示您同意修訂後的政策。",
      },
    ],
  },

  // Admin / Subadmin
  admin: {
    dashboard: "管理員儀表板",
    createUser: "建立用戶",
    allUsers: "所有用戶",
    role: "角色",
    status: "狀態",
    active: "啟用",
    disabled: "停用",
    creating: "建立中…",
    noUsers: "尚無用戶。",
    members: "會員",
    admins: "管理員",
    email: "電子郵件",
    password: "密碼",
    vipExpiry: "VIP到期時間",
    referralSource: "年齡範圍",
    createdAt: "創建於",
    actions: "操作",
    add: "添加",
    edit: "編輯會員",
    editAdmin: "編輯管理員",
    legacyEmailLogin: "舊帳戶仍可使用此電子郵件登入",
    save: "儲存",
    cancel: "取消",
    delete: "刪除",
    confirmDelete: "確定刪除此用戶？",
    ageRange: "年齡範圍",
    price: "價格",
    date: "日期",
    searchPlaceholder: "搜尋會員 (用戶名稱)...",
    noMembers: "尚無會員。",
    noAdmins: "尚無管理員。",
    previous: "上一個",
    next: "下一個",
    page: "頁面",
    daysLeft: "天",
    back: "返回",
    vip: "VIP",
    vipOnly: "僅顯示VIP",
    setVip: "設為VIP",
    vipDuration: "VIP時長",
    days3: "3天",
    days15: "15天",
    days30: "30天",
    days90: "90天",
    days365: "365天",
    analytics: "數據分析",
    totalUsers: "總用戶",
    vipUsers: "VIP用戶",
    dailyIncome: "當日收入",
    selectDate: "選擇日期",
    noIncome: "無收入",
    records: "記錄",
  },
  subadmin: {
    dashboard: "副管理員儀表板",
    addMember: "新增會員",
    members: "會員",
    noMembers: "尚無會員。",
  },

  // Records
  records: {
    title: "每日記錄",
    addNew: "新增記錄",
    date: "日期",
    description: "描述",
    media: "媒體",
    save: "儲存",
    cancel: "取消",
    delete: "刪除",
    readMore: "閱讀更多",
  },
};

// Recursively widen literal string types to `string`
type DeepStringify<T> = {
  [K in keyof T]: T[K] extends string ? string : DeepStringify<T[K]>;
};

export type TranslationKeys = DeepStringify<typeof zhTW>;
